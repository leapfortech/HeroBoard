...
public class Onboarding
{
    ...
    public Onboarding(int appUserId, int identityId, int addressId)
    {
        AppUserId = appUserId;
    ...
    public Onboarding(int id, int appUserId, long dpiFront, long dpiBack, long renap, long portrait, long address,
                      int identityId, int addressId, int renapIdentityId, String comment, int boardUserId,
    ...
    private static bool SetClassField<T>(ObdField field, T property, T newProperty)
    {
        if (field.Name[0] == '-')
            return false;

        PropertyInfo prop = typeof(T).GetProperty(field.Name);
        if (prop.PropertyType == typeof(String))
        {
            String str1 = (String)prop.GetValue(property);
            String str2 = (String)prop.GetValue(newProperty);
            if ((String.IsNullOrEmpty(str1) && String.IsNullOrEmpty(str2)) || str1 == str2)
                return false;
        }
        if (prop.PropertyType == typeof(int) && (int)prop.GetValue(property) == (int)prop.GetValue(newProperty))
            return false;
        if (prop.PropertyType == typeof(float) && (float)prop.GetValue(property) == (float)prop.GetValue(newProperty))
            return false;
        if (prop.PropertyType == typeof(double) && (double)prop.GetValue(property) == (double)prop.GetValue(newProperty))
            return false;
        if (prop.PropertyType == typeof(DateTime) && (DateTime)prop.GetValue(property) == (DateTime)prop.GetValue(newProperty))
            return false;

        prop.SetValue(property, prop.GetValue(newProperty));
        return true;
    }

    public bool SetDpiFrontCheck(ObdField field)
    {
        return field.Check == null ? false : SetDpiFrontCheck(field.Idx, field.Check.Checked);
    }

    public bool SetDpiBackCheck(ObdField field)
    {
        return field.Check == null ? false : SetDpiBackCheck(field.Idx, field.Check.Checked);
    }

    public bool SetIdentityField(ObdField field, Identity identity, Identity newIdentity)
    {
        return SetClassField(field, identity, newIdentity);
    }

    public int GetPortraitValue(bool photo)
    {
        return GetPortraitCheck(photo ? 0 : 1);
    }

    public bool SetPortraitValue(bool photo, int value)
    {
        return SetPortraitCheck(photo ? 0 : 1, value);
    }

    public bool SetAddressCheck(ObdField field)
    {
        return field.Check == null ? false : SetAddressCheck(field.Idx, field.Check.Checked);
    }

    public bool SetAddressField(ObdField field, Address address, Address newAddress)
    {
        return SetClassField(field, address, newAddress);
    }

    public bool SetBankAccountField(ObdField field, BankAccount bankAccount, BankAccount newBankAccount)
    {
        return SetClassField(field, bankAccount, newBankAccount);
    }
    ...
    {
        if (GetResultType(GetDpiFrontResult()) == 3)
            return true;
        if (GetResultType(GetDpiBackResult()) == 3)
            return true;
        if (GetResultType(GetRenapResult()) == 3)
            return true;
        if (GetResultType(GetPortraitResult()) == 3)
            return true;
        if (GetResultType(GetAddressResult()) == 3)
            return true;
        return false;
    }
}